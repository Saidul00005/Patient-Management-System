...
class TokenRefreshLifetimeSerializer(TokenRefreshSerializer):
    ...
    def validate(self, attrs):
        ...
        decoded_jwt = jwt_decode(str(access), verify=False, key=settings.SECRET_KEY, algorithms=['HS256'])
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def decode_token(token):
        decoded_token = jwt_decode(token, verify=False, key=settings.SECRET_KEY, algorithms=['HS256'])
